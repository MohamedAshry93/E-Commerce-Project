...
export default function WishList() {
  ...
  return (<>
    <div className="w-75 mx-auto my-5 pt-5 px-3 bg-main-light">
      ...
      {wishListDetails?.count !== 0 ? (<> {wishListDetails?.data.map((list) => <div key={list._id} className='row align-items-center border-bottom py-2'>
        ...
        <div className="col-md-10">
          <div className='d-flex align-items-center justify-content-between'>
            <div>
              <h3 className="h5 fw-bold">{list.title.split(' ').slice(0, 10).join(' ')}</h3>
              <h6 className="text-main">Price: {list.price} EGP</h6>
            </div>
            <div>
              <button onClick={() => addProductToCart(list.id)} className='btn bg-main text-white'>Add to Cart</button>
            </div>
          </div>
          <button onClick={() => removeItem(list.id)} className="btn p-0"><i className="fas fa-trash-can font-sm text-danger"></i> Remove</button>
        </div>
      </div>)}
      </>) : (<>
